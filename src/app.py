...
# 1. [System] 레이아웃 및 폰트 절대 고정 (v8.3 Ultimate)
...
# 2. [UI/UX] 불사신 CSS (v8.3 리스크 게이지 및 블랙아웃 락다운)
st.markdown("""
    <style>
    ...
    /* 2. [v8.3 핵심] 블랙아웃 버튼 락다운 (흰색 버튼 박멸) */
    .stButton>button, .stPopover>button, div[data-testid="stPopover"] button { 
    ...
    .gauge-fill { height: 100%; border-radius: 10px; transition: width 0.8s cubic-bezier(0.4, 0, 0.2, 1); }
...
# 3. [Data] 데이터 무결성 엔진 (Issuer Protection)
...
st.markdown("<h2> 📊 하이퍼 ETF 가디언 <span style='font-size:12px;color:#39FF14;'>[v8.3 Ultimate Integrity]</span></h2>", unsafe_allow_html=True)

ai_rep = get_ai_intel(f"유닛: {len(portfolio)}. 게이지 재건 및 무결성 데이터 엔진 가동.")
...
with tabs[1]: # Upcoming (인터랙션 완결)
    ...
    for i, itm in enumerate(upcs):
        ...
        with u_cols[i % 4]:
            ...
            if is_res:
                with st.popover("🚨 예약 완료 ∨"):
                    st.write("구매 예약을 취소하시겠습니까?")
                    if st.button("예, 취소합니다", key=f"can_{itm['ticker']}"): handle_action(itm, "CANCEL")
            else:
...
with tabs[2]: # Risk Control (게이지 가동 + 무결성 데이터)
    st.markdown("<div class='beta-notice' style='border-color:#FF3131;color:#FF3131;'><b>beta:</b> 원칙(-10.0%) 이탈 즉시 <b>'자동 매도'</b>하는 방어 시스템을 제공합니다.</div>", unsafe_allow_html=True)
    if not portfolio: st.info("추적 중인 자산이 없습니다.")
    else:
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("### 🛰️ 실시간 추적 (게이지 가동)")
                ...
                st.markdown(f"""
                    <div class='v8-box' style='border-left: 5px solid {g_color}; padding: 15px;'>
                        ...
                        <div style='display:flex;justify-content:space-between;margin-top:5px;'>
                            <small style='color:#8B949E;'>원칙(-10%) 도달률</small>
                            <small style='color:{g_color};font-weight:700;'>{gauge_val:.0f}%</small>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
        ...
        with c2:
            ...
            for p in res_p:
                ...
                """, unsafe_allow_html=True)
                with st.popover("🚨 예약 완료 ∨", use_container_width=True):
                    st.write("정말 구매 예약을 취소 하시겠습니까?")
                    if st.button("예, 취소합니다", key=f"can_res_{p['symbol']}"): handle_action(p, "CANCEL")

st.markdown(f"<div style='text-align:center;margin-top:50px;font-size:10px;color:#484F58;'>Hyper ETF Guardian v8.3 | Eternal Integrity | 19h Miracle</div>", unsafe_allow_html=True)