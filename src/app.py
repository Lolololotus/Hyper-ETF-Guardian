...
# 1. [System] 레이아웃 절대 고정 (v8.0 Final)
...
st.markdown("""
    ...
    /* 3. v8.0 칼정렬 및 버튼 블랙아웃 (테마 무조건 무시) */
    div.stButton > button, div.stPopover > button, .st-emotion-cache-19rxjzo { 
    ...
    /* 4. BETA 알림 박스 (Automation 시스템 예고) */
    .beta-box { 
...
# --- Main Dashboard ---
st.markdown("<h2> 📊 하이퍼 ETF 가디언 <span style='font-size:12px;color:#39FF14;'>[v8.0 최종 제출 마스터]</span></h2>", unsafe_allow_html=True)
portfolio = load_j(P_FILE)
etfs = load_j(ETF_FILE)
upcs = sorted(load_j(UPC_FILE), key=lambda x: x.get('listing_date', '9999-12-31'))

ai_rep = get_ai_intel(f"유닛: {len(portfolio)}. 무결성 검수 및 BETA 자동화 시스템 배치 완료.")
    ...
    for idx, th in enumerate(themes):
        with all_c[idx]:
            ...
            for r, itm in enumerate(tp[:10]):
                ...
                with c_row[0]:
                    st.markdown(f'<div class="list-row"><span style="color:#8B949E;width:15px;font-size:10px;">{r+1}</span><span class="issuer-name">{itm["issuer"]}</span><span class="product-name">{itm["name"]}</span></div>', unsafe_allow_html=True)
                with c_row[1]:
                    if st.button("해제" if is_t else "추적", key=f"tk_{idx}_{itm['symbol']}"): handle_action(itm, "TOGGLE")
            st.markdown("</div>", unsafe_allow_html=True)

with tabs[1]: # Upcoming (BETA Notice + Confirmation)
    st.markdown("""
        <div class="beta-box">
...
with tabs[2]: # Risk Control (BETA Notice + Split Columns)
    st.markdown("""
        <div class="beta-box" style="border-color: #FF3131; color: #FF3131; background: rgba(255, 49, 49, 0.05);">
            <b>beta : 현재 버전은 BETA 모드입니다.</b> 추후 정식 업데이트를 통해 증권사 계좌와 직접 연동, 
    ...
    with col_a:
        ...
        for p in live_p:
            ...
            st.markdown(f'<div class="v7-box" style="border-left: 5px solid {"#FF3131" if l_r <= -10 else "#39FF14"}; padding: 15px;"><div style="display:flex;justify-content:space-between;align-items:center;"><div><span class="issuer-name">{p.get("issuer", "Unknown")}</span><br><span class="product-name">{p["name"]}</span></div><div style="text-align:right;"><span style="color:{"#FF3131" if l_r < 0 else "#39FF14"}; font-size:18px; font-weight:900;">{l_r:+.2f}%</span></div></div></div>', unsafe_allow_html=True)
    ...
    with col_b:
        ...
        for p in res_p:
            st.markdown(f'<div class="v7-box" style="border-left: 5px solid #FFD700; padding: 15px;"><div style="display:flex;justify-content:space-between;align-items:center;"><div><span class="issuer-name">{p.get("issuer", "Unknown")} | {p.get("date", "Unknown")}</span><br><span class="product-name">{p["name"]}</span></div><div style="text-align:right;"><span style="color:#FFD700; font-size:16px; font-weight:900;">{p.get("qty",0)}주</span></div></div></div>', unsafe_allow_html=True)
            with st.popover("🚨 구매 예약 완료", use_container_width=True):
                st.write("정말 예약을 취소하시겠습니까?")
                if st.button("예, 취소합니다", key=f"can_res_{p['symbol']}"): handle_action(p, "CANCEL")

st.markdown(f"<div style='text-align:center;margin-top:50px;font-size:10px;color:#484F58;'>Hyper ETF Guardian v8.0 | Final Submission | 19h Miracle</div>", unsafe_allow_html=True)