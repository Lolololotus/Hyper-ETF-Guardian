...
# 1. [System] 레이아웃 절대 고정 (v8.1 Final)
...
# 2. [UI/UX] 불사신 CSS (v8.1 White-out 방어막)
st.markdown("""
    <style>
    /* 1. 불필요 요소 완전 숙청 */
    ...
    /* 3. [v8.1 핵심] 모든 버튼 시각적 암전 강제 고정 (Blackout Lockdown) */
    ...
    /* 호버 시 색상 반전 */
    button[kind="secondary"]:hover, button[kind="primary"]:hover, .stButton>button:hover, div[data-testid="stPopover"] button:hover { 
...
# --- Main Dashboard ---
st.markdown("<h2> 📊 하이퍼 ETF 가디언 <span style='font-size:12px;color:#39FF14;'>[v8.1 최종 무결성 마스터]</span></h2>", unsafe_allow_html=True)
portfolio = l_j(P_FILE)
all_etfs = l_j(ETF_FILE)
upcs = sorted(l_j(UPC_FILE), key=lambda x: x.get('listing_date', '9999-12-31'))

ai_rep = get_ai_intel(f"유닛: {len(portfolio)}. 지휘소 무결성 및 화이트아웃 숙청 완료.")
...
with tabs[1]: # Upcoming (인터랙티브 예약 시스템 + BETA Notice)
    st.markdown("""<div class="beta-notice"><b>beta : 현재 버전은 BETA 모드입니다.</b> 추후 정식 업데이트를 통해 증권사 계좌와 직접 연동, 예약한 종목을 상장 즉시 <b>'0.1초 자동 매수'</b>하는 풀-오토 시스템을 제공할 예정입니다.</div>""", unsafe_allow_html=True)
    u_cols = st.columns(4)
    for i, itm in enumerate(upcs):
        ...
        with u_cols[i % 4]:
            ...
            if is_res:
                with st.popover("🚨 구매 예약 완료"):
                    st.write("구매 예약을 취소하시겠습니까?")
                    if st.button("예, 구매 예약을 취소 하시겠습니까?", key=f"can_upc_{itm['ticker']}"): handle_action(itm, "CANCEL")
            else:
...
with tabs[2]: # Risk Control (칼럼 분리 + BETA Notice)
    st.markdown("""<div class="beta-notice" style="border-color:#FF3131;color:#FF3131;background:rgba(255,49,49,0.05);"><b>beta : 현재 버전은 BETA 모드입니다.</b> 추후 정식 업데이트를 통해 증권사 계좌와 직접 연동, 당신의 자산이 원칙(-10.0%)을 이탈한 즉시 <b>'자동 매도'</b>하는 Full-Auto 방어 시스템을 제공할 예정입니다.</div>""", unsafe_allow_html=True)
                ...
                st.markdown(f"""
                    <div class="v7-box" style="border-left: 5px solid {'#FF3131' if l_r <= -10 else '#39FF14'}; padding: 15px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;">
                            ...
                            <div style="text-align:right;"><span style='color:{'#FF3131' if l_r < 0 else '#39FF14'}; font-size:18px; font-weight:900;'>{l_r:+.2f}%</span></div>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
...
st.markdown(f"<div style='text-align:center;margin-top:50px;font-size:10px;color:#484F58;'>Hyper ETF Guardian v8.1 | Mission Finalized | 19h Miracle</div>", unsafe_allow_html=True)